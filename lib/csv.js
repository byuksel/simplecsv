...
csv.prototype.csvdataToJSON = function(csvdata) {
  ...
  if (columnnames.length === 0 && csvdata.columnCount !== 0) {
    var digitCount = csvdata.columnCount.toString().length;
    // No columnnames, let's fill it up with generated names
    for (var m = 0; m < csvdata.columnCount; m++) {
      var neededZeroCount = digitCount - m.toString().length;
      var columnName = 'Col ' + Array(neededZeroCount + 1).join('0') + m.toString();
